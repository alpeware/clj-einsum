...
(defn gemma2-alias-resolver
  "Returns a resolver function mapping Gemma 2 Tensor Logic AST variable keywords
   to canonical HuggingFace Safetensors parameter paths under `prefix-base`."
  ([]
   (gemma2-alias-resolver "model."))
  ([prefix-base]
     ...
     (fn [k]
       (let [k-str (if (keyword? k) (name k) (str k))]
         (cond
           ...
           :else
           (if-let [[_ prefix idx-str] (re-matches #"^(input_ln_w|q_w|k_w|v_w|o_w|post_attn_ln_w|pre_mlp_ln_w|post_mlp_ln_w|gate_w|up_w|down_w)_(\d+)$" k-str)]
