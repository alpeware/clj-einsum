(ns einsum.logic.interpret-diff-test
  "Differential tests: the JVM interpreter (einsum.logic.interpret) must agree
  ...
  (:require [einsum.logic.interpret :as interp]
...
(deftest diff-dynamic-slice
  (let [graph {:name "diff_ds"
               :invars [[:x [:tensor [4 4] :f32]] [:start [:tensor [1] :i32]]]
               :outvars [:y]
               :eqns [{:op :stablehlo/dynamic_slice
                       :invars [:x :start]
                       :outvars [:y]
                       :attrs {:start_indices [:start 1] :slice_sizes [2 2]}}]}
        xs (mapv float (range 16))
        starts [1]]
    (assert-parity! graph
                    {:x {:shape [4 4] :dtype :f32 :data xs}
                     :start {:shape [1] :dtype :i32 :data starts}}
                    [2 2] 1e-6)))
