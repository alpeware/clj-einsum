(ns tools.train-mem-projection
  "Learned memory projection (frozen-LLM linear probe via autodiff).
   ...
   Dogsfoods einsum.logic.autodiff/derive-adjoint-equations to implement the backward contraction:
     dL/dW = h^T ⊗ dL/du"
  (:require [einsum.core :as xla]
            [einsum.logic.autodiff :as autodiff]
            ...
            [tools.poc-span-probe :as span-base])
  (:gen-class))
...
(defn run-training-pipeline [opts]
  ...
  (let [triples-file (io/file (:triples opts))]
    ...
    (let [kb-data (edn/read-string (slurp triples-file))
          ...
          _ (println "------------------------------------------------------------------")
          _ (println " Dogfooding einsum.logic.autodiff/derive-adjoint-equations:")
...
(defn -main [& args]
  (let [opts (parse-cli-args args)]
    (if (g4/needs-libjsig-reexec? opts)
      (g4/reexec-with-libjsig! args "tools.train-mem-projection")
      (do
