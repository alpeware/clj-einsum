(ns tools.benchmark-ternary
  "Benchmark and evaluation suite for CAT-Q (1.58b ternary quantization) in OpenXLA PJRT.
        ...
        (vec (mapcat
              (fn [i]
                (let [cfg (if (seq layer-configs) (nth layer-configs i nil) nil)
                      is-global? (if cfg (:is-global? cfg) (contains? #{2 5 8 11 14 17 20 23 26 29 32} i))
...
(defn evaluate-syntax
  ...
  (try
    (let [form-match (re-find #"\([a-zA-Z0-9\-\?\!\_]+[\s\S]*\)" text)]
      (if form-match
        (let [parsed (read-string form-match)]
          {:valid-syntax? (some? parsed) :form form-match})
...
(defn render-summary-report
  ...
  (let [{:keys [gate1 gate2 gate3 gate4 model-name backend]} experiment-report]
    (str
     ...
     "    • Target Ceiling                 : < 1,800.0 seconds (30 minutes)\n"
     ...
     "----------------------------------------------------------------------------------------------------\n"
     "  GATE 4 (Continuous Recursion & Judgment Derivative):\n"
  ...
  (let [opts (gemma4-inf/parse-cli-args args)]
    (if (gemma4-inf/needs-libjsig-reexec? opts)
      (do
        ...
        (gemma4-inf/reexec-with-libjsig! args "tools.benchmark-ternary"))
      (do
